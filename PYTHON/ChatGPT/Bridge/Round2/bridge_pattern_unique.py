...
class TV(Device):
    ...
    def set_volume(self, volume):
        self._volume = max(0, min(volume, 100))  # ให้เสียงอยู่ในช่วง 0 ถึง 100
...
def main():
    ...
    # ทดสอบรีโมทแบบพื้นฐานกับ TV
    remote.power()
    print(f"TV เปิดอยู่: {tv.is_enabled()}")
    remote.volume_up()
    print(f"เสียง TV หลังจากเพิ่ม: {tv.get_volume()}")

    # ทดสอบรีโมทขั้นสูงกับ TV
    advanced_remote.mute()
    print(f"เสียง TV หลังจากปิดเสียง: {tv.get_volume()}")
    ...
    print(f"เสียง Radio หลังจากลด: {radio.get_volume()}")
